...
int safeRecvfrom(int recv_sk_num, uint8_t * packet, int len, Connection * from)
{
	...
	if ((recv_len = recvfrom(recv_sk_num, packet, len, 0, (struct sockaddr *) &(from->remote),
	&from->len)) < 0)
	{
...
{
	// nameOfAddress is just used to print out "Client" or "Server" or whatever you want on
	//your output
